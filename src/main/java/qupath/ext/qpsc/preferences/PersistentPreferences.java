...
public class PersistentPreferences {
    ...
    // ================== PROPAGATION SIFT REFINEMENT ==================
    // Independent SIFT settings used by the post-propagation refinement pass
    // (see PropagationManagerDialog). Shares the same algorithm as the
    // single-tile-refinement SIFT but is tuned for a different image-pair
    // context: base WSI (e.g. OCUS40 H&E) vs the user-selected reference
    // sub-acquisition (e.g. PPM 90deg). Stored under a separate property
    // namespace so users can dial in propagation matching without disturbing
    // the camera-vs-WSI defaults used during alignment.
    ...
    // reference (e.g. angle=90.0 for PPM, or modality=BF_color for OWS3).
    // Stored as (key, value) so the worker can match `entry.metadata[key]
    // == value` to find the reference sub in each group.
