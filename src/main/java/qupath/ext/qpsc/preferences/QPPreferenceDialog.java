    ...
    public static void installPreferences(QuPathGUI qupath) {
        ...
        items.add(new PropertyItemBuilder<>(multiSlideAcquisitionModeProperty, String.class)
                ...
                .description("How much of the multi-slide SETUP pass runs without a human.\n"
                        ...
                        + "\n"
                        + "Built and working: every tile pick is made for you (reference tiles are\n"
                        + "chosen by texture and spread, away from the tissue edge), multi-tile\n"
                        + "refinement drives its own Select tile / Solve steps, per-slide reset, and\n"
                        + "auto-confirm on the annotation, refinement-choice, landmark-confirm and\n"
                        + "acquisition-start dialogs.\n"
                        + "\n"
                        + "Where it still stops and asks: a slide whose tissue is too small to hold\n"
                        + "an interior tile, and a SIFT match that comes back below the confidence\n"
                        + "threshold -- accepting either without a human would mis-align the slide.\n"
                        + "Those hand back THAT SLIDE only and send a push notification (configure\n"
