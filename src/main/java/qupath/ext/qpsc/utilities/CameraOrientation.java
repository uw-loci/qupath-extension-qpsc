 ...
 * double-click-to-centre, AND a small 2x2 stitched acquisition. If only the
...
public enum CameraOrientation {
    ...
    /** Rotated 180 deg (upside down). Equivalent to {@link #FLIP_H} composed with {@link #FLIP_V}. */
    ROT_180(-1, 0, 0, -1),

    /** Rotated 90 deg clockwise. Sample +X appears at display -Y (top); sample +Y appears at display +X (right). */
    ROT_90_CW(0, 1, -1, 0),

    /** Rotated 90 deg counter-clockwise (i.e. 270 deg clockwise). */
    ROT_90_CCW(0, -1, 1, 0),
    ...
    public double[] sampleToDisplay(double dsx, double dsy) {
        return new double[] {m00 * dsx + m01 * dsy, m10 * dsx + m11 * dsy};
    }
    ...
    public double[] displayToSample(double ddx, double ddy) {
        return new double[] {m00 * ddx + m10 * ddy, m01 * ddx + m11 * ddy};
    }
