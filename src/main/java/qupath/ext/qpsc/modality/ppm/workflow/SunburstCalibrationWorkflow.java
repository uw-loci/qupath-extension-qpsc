...
public class SunburstCalibrationWorkflow {
        ...
        Platform.runLater(() -> {
            try {
                ...
                Dialogs.showErrorMessage(
                        "Calibration Error", "Failed to start sunburst calibration: " + e.getMessage());
            }
        });
    }
        ...
        CompletableFuture.runAsync(() -> {
                ...
                .exceptionally(ex -> {
                    ...
                    Platform.runLater(() -> {
                        ...
                        Dialogs.showErrorMessage(
                                "PPM Calibration Error", "Failed to execute calibration: " + ex.getMessage());
                    });
                    return null;
                });
    }
    ...
    private static void executeCalibration(SunburstCalibrationParams params, Alert progressDialog) {
        ...
        try {
            ...
            Platform.runLater(() -> {
                progressDialog.close();
                CalibrationResultDialog.showResult(
                        resultData, SunburstCalibrationWorkflow::run, finalCenterRetry, finalTuneThresholds);
            });
            ...
            Platform.runLater(() -> {
                progressDialog.close();
                CalibrationResultData errorResult =
                        CalibrationResultData.failure(e.getMessage(), null, null, params.outputFolder());
                CalibrationResultDialog.showResult(errorResult, SunburstCalibrationWorkflow::run, null, null);
            });
        }
    ...
    private static Runnable buildTuneThresholdsCallback(SunburstCalibrationParams params, String imagePath) {
        return () -> {
            ThresholdPreviewDialog.showDialog(imagePath, params.saturationThreshold(), params.valueThreshold())
                    .thenAccept(thresholdResult -> {
                        ...
                        logger.info(
                                "Re-running calibration with tuned thresholds: sat={}, val={}",
                        ...
                        Platform.runLater(() -> {
                            ...
                            Label progressLabel = new Label(String.format(
                                    "Re-analyzing with tuned thresholds (sat=%.3f, val=%.3f)...",
     ...
     */
    private static void executeCalibrationWithExistingImage(
            SunburstCalibrationParams params, String imagePath, Alert progressDialog) {
        try {
            String configFileLocation = QPPreferenceDialog.getMicroscopeConfigFileProperty();
            MicroscopeSocketClient socketClient =
                    MicroscopeController.getInstance().getSocketClient();
            ...
            Platform.runLater(() -> {
                progressDialog.close();
                CalibrationResultDialog.showResult(
                        resultData, SunburstCalibrationWorkflow::run, centerRetryCallback, tuneThresholdsCallback);
            });
            ...
            Platform.runLater(() -> {
                progressDialog.close();
                CalibrationResultData errorResult =
                        CalibrationResultData.failure(e.getMessage(), imagePath, null, params.outputFolder());
                ...
                CalibrationResultDialog.showResult(
                        errorResult, SunburstCalibrationWorkflow::run, retryCallback, tuneCallback);
            });
        }
    }
     ...
     */
    private static void retryWithCenter(SunburstCalibrationParams params, String imagePath, int centerY, int centerX) {
        ...
        Platform.runLater(() -> {
            ...
            Label progressLabel = new Label(
                    String.format("Re-analyzing with manually selected center (Y=%d, X=%d)...", centerY, centerX));
            progressLabel.setStyle("-fx-font-size: 12px;");
            ...
            CompletableFuture.runAsync(() -> {
                        try {
                            ...
                            String resultImagePath =
                                    resultData.imagePath() != null ? resultData.imagePath() : imagePath;
                            CalibrationResultDialog.CenterRetryCallback centerRetryCallback =
                                    (cy, cx) -> retryWithCenter(params, resultImagePath, cy, cx);
                            Runnable tuneThresholdsCallback = buildTuneThresholdsCallback(params, resultImagePath);
                    ...
                    .exceptionally(ex -> {
                        ...
                        Platform.runLater(() -> {
                            progressDialog.close();
                            Dialogs.showErrorMessage(
                                    "PPM Calibration Error", "Failed to retry calibration: " + ex.getMessage());
                        });
                        return null;
                    });
        });
    ...
    private static CalibrationResultData parseCalibrationResult(String json) {
        try {
            ...
            String imagePath =
                    result.has("image_path") && !result.get("image_path").isJsonNull()
                            ? result.get("image_path").getAsString()
                            : null;
            String maskPath =
                    result.has("mask_path") && !result.get("mask_path").isJsonNull()
                            ? result.get("mask_path").getAsString()
                            : null;

            if (success) {
                double rSquared =
                        result.has("r_squared") ? result.get("r_squared").getAsDouble() : 0.0;
                int spokesDetected = result.has("spokes_detected")
                        ? result.get("spokes_detected").getAsInt()
                        : 0;
                String plotPath =
                        result.has("plot_path") ? result.get("plot_path").getAsString() : null;
                String calibrationPath = result.has("calibration_path")
                        ? result.get("calibration_path").getAsString()
                        : null;
                ...
                return CalibrationResultData.success(
                        rSquared, spokesDetected, plotPath, calibrationPath, imagePath, maskPath, warnings);
            } else {
                ...
                String outputFolder = result.has("output_folder")
                        ...
                        ? result.get("output_folder").getAsString()
                        : null;
            ...
            return CalibrationResultData.failure(
                    "Failed to parse server response: " + e.getMessage(), null, null, null);
        }
    }
}
