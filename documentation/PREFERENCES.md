...
### Macro image pixel size in microns
...
**It is slower.** The tile positions are solved once per annotation, and every angle/channel then reuses that solve. For angles, the reference angle is stitched first and solves as it goes, so expect it to take noticeably longer than the others. For fluorescence channels, the solve runs before any channel is stitched, and then all channels stitch in parallel.

**Which target is solved on matters.** Registration matches the *image* content in each overlap, so it is only as good as the contrast in that image -- and sibling angles of the very same tissue can differ enormously. PPM's crossed angles sit near extinction: dark and low-contrast pictures of tissue that is perfectly well textured. QPSC therefore solves on the **90-degree (open) angle** whenever it was acquired, and falls back to the first target otherwise. Fluorescence solves on the channel(s) ticked in the channel picker's **Align** column. With none ticked it uses the focus channel, and with no usable focus channel a normalized merge of every acquired channel. See [Tile alignment channel](CHANNELS.md#tile-alignment-channel).
...
**Approach-from-safe-Z (when validation is available).** If a [Focus Approach Validation](AUTOFOCUS.md#focus-approach-validation) run has been completed and is still valid for the modality/objective combination, slot-jump autofocus uses the safer **approach-from-safe-Z** strategy instead of the edge-retry walk. The objective retracts to the measured safe Z and scans in once, bounded by the measured approach distance. This is the more dangerous option, not the safer one: it traverses the whole distance toward the sample rather than creeping near the last focus, from a hand-entered starting position. It buys bounded, auditable travel, and only pays off with a physically verified safe Z for the insert in use and a passing validation run. Accuracy relative to the walk is untested on hardware so far.

**Extended attempt budget (when validation is unavailable).** Because a fresh slide's true focus depends on its mounting-media thickness and can sit well beyond one narrow sweep from the previous slide's Z, the slot-jump streaming search uses an **extended attempt budget** (6 edge-retry attempts, versus the default 3) so its narrow-step focus march can reach a far focus without widening the per-sweep range. This is specific to the slide-change case and does not change the global autofocus sweep/search ranges. Extra attempts cost time only when focus is actually far; a near-seed scan still commits on the first attempt. When approach-from-safe-Z is available, the extended attempt budget is not used.
