...
**Approach-from-safe-Z (when validation is available).** If a [Focus Approach Validation](AUTOFOCUS.md#focus-approach-validation) run has been completed and is still valid for the modality/objective combination, slot-jump autofocus uses the safer **approach-from-safe-Z** strategy instead of the edge-retry walk. The objective retracts to the measured safe Z and scans in once, bounded by the measured approach distance. This replaces the open-ended edge-retry search with a controlled scan: travel is bounded and the direction is measured rather than inferred. Accuracy relative to the edge-retry walk is untested on hardware so far.
