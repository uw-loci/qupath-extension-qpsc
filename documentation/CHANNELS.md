...
- **Axial orientation**: The output orientation map is axial: 0 and 180 degrees represent the same physical orientation. Standard averaging and interpolation (e.g. bilinear downsampling, seam blending) produce invalid results on axial data. Downstream tools must apply angle-doubling transforms (sin(2θ)/cos(2θ)) during downsampling and seam blending. Image mirroring negates the angle and must be tracked. The acquired tiles declare axial orientation in their OME metadata; any external tool processing them must honour that declaration.
- **Extinction-safe autofocus**: The extinction state (`State0`) is near-black by construction -- it is the state the calibration drove to minimum transmission -- so it is never a usable autofocus reference. The picker's default focus channel comes from `ModalityHandler.defaultFocusChannelId(...)`, which returns library order by default and which `LCPolScopeModalityHandler` overrides to return the first non-extinction state. The user can still pick any swing state. See [AUTOFOCUS.md](AUTOFOCUS.md#focus-channel-interaction).
...
- Python server -- `acquisition/workflow.py` re-resolves the plan from YAML, moves the `--focus-channel` entry to position 0, then for each tile iterates every channel: `apply_channel_hardware_state` (including any per-run intensity overrides from `--channel-intensities`) -> set exposure -> snap -> write to `{annotation}/{channel_id}/{tile}.tif`. **Z-stack support (`e8e3799`):** when the acquisition has Z-stack enabled, the channel loop iterates Z planes per channel (channel-outer, Z-inner ordering minimizes filter wheel changes), accumulates planes, and applies the configured projection per channel. Output: one projected file per `{annotation}/{channel_id}/{tile}.tif`; with `save_raw_tiles=true` also `{annotation}/{channel_id}/z000/{tile}.tif`, `z001/`, etc. Stage Z is restored to center between channels. Saturation runaway detection fires once per tile per channel.
- **Loop-order toggle (2026-05-14):** the per-tile nest can be flipped to z-outer / channel-inner via `--inner-axis channel` (the acquisition dialog exposes this as "Channels per Z (drift-tolerant)" inside the Z-stack section). In z-outer mode every channel re-images at each z plane before z advances -- channels x z_planes filter switches per tile (slower) but tightly z-registered across channels, useful for live samples or workflows that need per-z channel correlation. Default stays at channel-outer / z-inner. See `documentation/tools/z-stack-timelapse.md` for the user-facing description and `documentation/developer/SOCKET_PROTOCOL.md` for the flag.
...
1. **Per-channel stitching (parallel).** `StitchingHelper.stitchChannelDirectories` dispatches all acquired channel ids for concurrent stitching, bounded by the **Stitching concurrency** preference (default: 4 concurrent writers). Each channel's stitching call produces a single-channel OME-TIFF pyramid on disk; channels stitch independently so they can run in parallel. The former sequential behavior for OME-TIFF (a BioFormats concurrency limitation) no longer applies with the `DirectTiffOutputWriter`. Because the QuPath project tree would otherwise fill up with N intermediates per annotation, the stitching params for this stage set `skipProjectImport: true` -- the per-channel pyramids land in the output directory but are not added to the project. Note: parallelism is bounded **within a single annotation**; different annotations stitch one at a time as the stage moves between acquisition regions.
