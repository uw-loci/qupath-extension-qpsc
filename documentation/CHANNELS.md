...
- **Extinction-safe autofocus**: The extinction state (`State0`) is near-black by construction -- it is the state the calibration drove to minimum transmission -- so it is never a usable autofocus reference. `LCPolScopeModalityHandler.defaultFocusChannelId(...)` computes the right default (the first non-extinction state), but it is **not yet wired into the picker**, whose fallback is still first-in-library-order. Until that is wired, the operator must select a swing state on the Focus radio. See [AUTOFOCUS.md](AUTOFOCUS.md#focus-channel-interaction).
