...
| Control | Description |
|---|---|
| **Autofocus** | Single button that runs either streaming continuous-Z autofocus or Sweep Autofocus, depending on your selection in the Autofocus Configuration dialog. **Left-click** to run autofocus. While Streaming AF is running the button reads **Cancel Autofocus** and can be clicked to abort. While Sweep Autofocus is running the button reads **Sweeping...** and is disabled (server-side operation has no client-side cancel; Z is returned to the starting position on any server-side failure). **Right-click** to open the Autofocus Configuration dialog (Extensions > QP Scope > Utilities > Image Quality > Autofocus Configuration) for method selection and parameter tuning. This is useful when you want to switch autofocus methods while the button is greyed out (e.g. during acquisition or between live frames). |
...
> **Slow-autofocus warning.** The first time you run autofocus in a session while the current exposure is above 15 ms (e.g. a fluorescence channel at 100 ms), a confirmation dialog appears noting that the Z scan will be slow and the live image may not change noticeably during it -- this is expected, not a freeze. Choose **Autofocus** to proceed or **Cancel**. The prompt appears at most once per session.
...
The Autofocus Configuration dialog (Extensions > QP Scope > Utilities > Image Quality > Autofocus Configuration) contains radio buttons labeled "Live Viewer Autofocus button uses:" that let you select which method the Live Viewer's Autofocus button runs:
...
| Z Focus (Vertical Bar Widget) | Visual Control | Two-bar Z navigation widget with inline focus-metric trace: |
| — Coarse Bar | Vertical Bar | Wide bar covering the full stage Z range (or a saved subset). Drag the thumb up/down to move Z across the whole accessible range. Scroll the mousewheel over either bar to stream Z by the current step size. Edit the range limits via the ellipsis (...) button. |
| — Fine Bar | Vertical Bar | Narrow bar centered on the current Z with a configurable half-width (default ±20 um). Provides precision control near the current focus plane. Scroll the mousewheel over either bar to stream Z by the current step size. Recenter the window via the ellipsis (...) button to adjust the half-width. |
| — Focus Trace | Line Graph | Semi-transparent curve drawn in the left margin of both bars, anchoring at the track's left edge and extending leftward proportionally to the Brenner gradient (focus-quality metric) sampled during live streaming. Each bar (coarse and fine) shows the trace scaled to its own Z range, revealing both the big-picture peak (coarse) and local focus shape (fine). The trace is location-specific; it clears automatically when you move the stage more than 1 um in XY. Higher curve extension = sharper focus. Useful for spotting slight focus drift. |
...
| — Max Z Focus Button | Button | Move Z to the position of the highest focus-metric sample in the current trace. Greyed out when no trace samples are present or when stage movement is disabled. Useful for quickly returning to the best-focus Z after exploring other positions. |
| — Z Step Control | TextField | Step size in micrometers used by every mousewheel scroll-as-focus-knob target: the Z position field, the Move Z button, the step field itself, and now the coarse and fine bars. |
...
| Save to Profile | Button | A caption under the button states what it does in one line ("makes these the system default"), since the distinction from a preset is not obvious. Writes the live lamp intensity to `acquisition_profiles.<profile>.illumination_intensity` in the microscope YAML and persists the live exposure as the Camera default. A confirmation dialog shows **all** the values being saved (intensity + exposure) before writing. Background Collection and tiled Acquisition pull the lamp intensity from the YAML profile, so save here to make a Camera-tab tune-up drive BG and acquisition. Brightfield has no profile-level exposure field -- exposure is carried as the persisted Camera default, not under `acquisition_profiles`. |
...
| Radio state vs. hardware | -- | The radios **and the status line above the channel grid** follow the instrument, not your last click. Anything that changes the light path elsewhere -- an acquisition, or a channel/focus test in the acquisition dialog -- updates them. After an acquisition the selection returns to **(None)** with "Light path changed elsewhere -- pick a channel to resume preview", because the run ends on its own last channel and the server switches channel illumination off during teardown (so a fluorescent sample is not left bleaching under a channel nobody is viewing). Selecting a radio programmatically does not touch hardware. The status line is rewritten on every state change, so a green "Applied: DAPI" cannot outlive the state it described. |
| Per-channel rows | Radio + Spinner + Spinner | One row per channel. Labels show the excitation wavelength first to match MicroManager's LED naming (e.g., "385 nm (DAPI)", "475 nm (FITC)"), since biologists key off the wavelength rather than the dye id. The wavelength is parsed from the channel's YAML `display_name`; channels without a wavelength (e.g. the Brightfield entry of a `bf_if` library) fall back to the bare id. The radio button drives hardware via `APPLYCH`: the cube/shutter presets run, the per-channel light source switches, the per-channel intensity property is written, and the per-channel exposure is applied -- all on the server in `apply_channel_hardware_state`, the same helper the acquisition workflow uses. The exposure spinner round-trips to the live camera; the intensity spinner persists to PersistentPreferences for the next acquisition (independent live tuning of the channel's intensity property requires SETILLMD on the channel's intensity device, scheduled if friction surfaces). |
| Intensity spinner range | -- | When the microscope is connected, the spinner is limited to the range Micro-Manager reports for that channel's `intensity_property` (GETPROPL), so a 0-100 LED will not accept 105. A value already outside the reported range is clamped and the status line says so. Offline, or for a property with no numeric limits, the spinner keeps its default wide range. |
| Channel Preset Bar | ComboBox + Save / Delete | Named presets capturing per-channel exposure and intensity spinner state. **Save** snapshots every channel's current spinner values and marks the currently-active preview channel as "selected" in the preset. **Selecting** a preset in the dropdown restores those spinner values across all channels and, if exactly one channel was selected, fires its radio (APPLYCH switches the cube/light path to it). **Delete** removes the preset. Presets are shared with the [Bounded Acquisition](bounded-acquisition.md) dialog's Fluorescence panel, so saving here makes the preset available there and vice versa; the dropdown re-reads the list when opened, so a preset saved in the other dialog appears without reopening this tab. The dropdown starts empty each session -- a preset name is shown only once you apply one, because the spinners themselves are seeded from hardware and profile values, not from a preset. |
| Save / Load Preset | Buttons | (Legacy, and **not shown for modalities with a channel library** -- those use the Channel Preset Bar above. It stores a single unified exposure plus gains, which cannot describe a multi-channel tab, and the gain read fails outright on a camera without gain.) Save: stores `profile\|exp\|gain\|illum` keyed by modality + objective + detector; on a camera that reports no gain the gain field is left empty and only exposure + illumination are restored. Load: runs `APPLYPR` for the saved profile *before* writing `SETCAM` and `SETILLM`, so the right cube + light path are in place when the preset's illumination power lands. |
