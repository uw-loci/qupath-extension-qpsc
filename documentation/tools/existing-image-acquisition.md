...
For **Multi-Tile Refinement,** a dialog guides you through adding reference points: for each tile you select, the stage automatically moves to its predicted position, SIFT attempts auto-alignment (or you may nudge manually), and the measured stage position is captured. The dialog displays the recovered rotation and scale live so you can diagnose whether the slide truly has play in the holder (non-zero rotation) or a residual pixel-size mismatch (scale far from 1.0). Once you have 2+ points, click "Solve & Save" to compute the correction and apply it to the alignment. Spread the reference tiles far apart (not in a line) for the best rotation estimate.
