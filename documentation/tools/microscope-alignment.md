...
**Multi-tile refinement panel:** For slides that may sit rotated in their slot (common with the multi-slide vertical holder), the **multi-tile** refinement mode shows a numbered-steps panel that solves a rotation + scale correction from 2 or more reference points. Each step is color-matched to its button -- **1. Select tile** (blue), **2. Auto-Align (SIFT)** (amber), **3. Add reference point** (teal), **4. Solve & Save** (green) -- and an attention pulse glows the next action. Spread the points far apart (not in a line) for the best rotation estimate. After the first tile is captured, the button is relabeled to "**Select a new tile**". It stays **disabled until exactly one tile is selected in the QuPath image and that tile has not already been captured** for this refinement -- so a dead click, or a pick that would be rejected after the stage had already moved, is not possible. (When auto-picked tiles are queued the button needs no selection, since the queue supplies the tile.)

**Fit quality, and why a third point matters:** A similarity transform has four degrees of freedom and each reference point supplies two equations, so a 2-point fit is *exactly determined*: it reproduces both points perfectly and its RMS residual is zero for any pair of points, however badly one was measured. The zero is arithmetic, not evidence, so the panel reports the RMS as **n/a** at 2 points rather than **0.0 um**. Any disagreement between two points has nowhere to go except rotation and scale -- meaning one mis-measured point does not look like an error, it looks like a rotated, rescaled slide. When a 2-point fit produces a rotation over 0.5 deg or a scale more than 0.3% from 1.0, the panel turns orange and asks for a third point, because a third point is what tells a genuinely rotated slide from a bad click: it makes the RMS measure something. This is **advisory, not a gate** -- **Solve & Save** stays enabled throughout, so an unattended batch is never stalled by it, and the warning is written to the log for later review.
...
**Cross-modality matching (16-bit camera vs 8-bit WSI, mono vs color reference):** When the microscope camera produces 16-bit images and the reference is an 8-bit H&E or PPM WSI, SIFT collapses if the camera range is naively bit-shifted to 8-bit. The SIFT Settings dialog exposes colour-to-grayscale conversion mode (`GREEN` / `LUMINANCE`) for colour reference images, mono-normalization mode (`PERCENTILE` / `MIN_MAX` / `BIT_SHIFT`), percentile clip points, and CLAHE settings. For H&E and colour brightfield references, `GREEN` (default) uses the green channel to preserve tissue-dark intensity polarity matching a monochrome camera; `LUMINANCE` is the legacy weighting. Defaults (colour: `GREEN`; bit-depth: `PERCENTILE` 2/98 + CLAHE on, clipLimit=2.0) are tuned for matching colour reference scans against a monochrome microscope camera. See [Preferences > SIFT Auto-Alignment](../PREFERENCES.md#sift-auto-alignment) for tuning guidance.
