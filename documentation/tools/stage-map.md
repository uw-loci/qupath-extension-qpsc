...
| Stage view | ComboBox | Auto | **Stage View only** — supplies the flip between Camera View and how the slide physically sits on the stage (inverted vs upright scope, optical flip). Set this by eye if Stage View does not match the slide's actual orientation on the bench. **Auto** (default) leaves Stage View identical to Camera View — the historical behavior, no change unless you explicitly set it. **Flip horizontal / Flip vertical / Rotate 180** apply the corresponding transform to Stage View only; Camera View is unaffected. Independent of which project image is open. |
