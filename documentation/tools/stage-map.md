...
| Camera View / Stage View | CheckBox | From config | Chooses how the Stage Map is oriented. **Checked = "Camera View"**: oriented like the Live Viewer and the acquisition image (what the camera sees). **Unchecked = "Stage View"**: oriented like the slides as they physically sit on the stage. The checkbox label changes with its state. Camera View applies the active scope's **Camera Orientation** only (stage polarity is already baked into the map geometry). The macro overlay's optical flip (flipMacroX/Y from the selected source preset) is applied to the macro image independently, then both are carried together into the displayed frame. Independent of which project image is open. Updates automatically when stage polarity or camera orientation are edited (e.g., via Calibrate Directions). Toggle to compare the two views. |
| Scope type | ComboBox | Upright | **Stage View only** — whether the objective faces up (Upright) or down (Inverted). Part of the per-microscope hardware description that determines how the slide physically sits on the stage. Read from and written to the microscope config's `light_path: scope_type` block. Defaults to Upright (identity orientation). |
| Slide insertion | ComboBox | A | **Stage View only** — the two ways a slide drops into the holder (slide can be inserted either way, differing by an in-plane 180-degree rotation). Read from and written to the microscope config's `light_path: slide_insertion` block. Defaults to A (identity orientation). |
| Inverted flip axis | ComboBox | Vertical | **Stage View only, inverted scopes only** — which physical axis the slide is turned over about to face the coverslip down (Vertical = the long axis, Horizontal = the short axis). Ignored on upright scopes. Read from and written to the microscope config's `light_path: inverted_flip_axis` block. Defaults to Vertical. |
| Overlay Macro | CheckBox | Auto | Overlay the current macro image on the map display. Automatically enabled when a macro image and alignment transform are detected. Requires a Preset to be selected. Also updates when switching between images in the project. **Note:** Disabled for multi-slot slide holders (e.g., quad_v) because the overlay would incorrectly span all slots — a single macro belongs to one slot only. Single-slide inserts are unaffected. **Live preview during Multi-Slide Assignment:** When the Multi-Slide Assignment Dialog is open (for `quad_v` or similar carriers), the Stage Map temporarily displays a live preview of all assigned slides at their chosen rotations, overlaid on the holder's slots. The Multi-Slide Assignment Dialog includes a **Source scanner** dropdown at the top where you can select which microscope (scanner) produced the macro images—this source is stamped onto every assigned slide that lacks it. The preview updates in real time as you change slide assignments, rotations, and source scanner selection, and follows the canvas during zoom/pan so you can inspect the layout at any scale. The preview remains visible after the dialog closes, serving as a layout reference through the per-slide alignments that follow. If you switch the Stage Map to a *different* carrier insert, the preview clears (the rotated layout is specific to that carrier); if you re-select the same carrier, the preview is preserved. |
