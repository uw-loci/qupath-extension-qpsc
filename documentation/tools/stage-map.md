...
| Calibrate... | Button | — | **New:** Calibrate the selected insert's reference points by driving the stage to each point in Live View and capturing the position. Opens a dialog showing the current stage coordinates; press "Use current" to capture each axis (e.g. well-edge aperture left/right/top/bottom for petri-dish carriers). Values are saved directly back to the microscope YAML configuration file and the map reloads automatically. Only enabled for inserts that have editable reference points in the config (the synthesized single-slide baseline cannot be calibrated this way). |
...
- **Calibrating custom stage inserts (e.g. petri-dish carriers):** Select the insert in the **Insert Configuration** dropdown, then click **Calibrate...** to define its reference points interactively. Drive the stage so the objective centers on a key feature (e.g. the well's left edge on a petri dish), then click "Use current X" to capture that position. Repeat for all required points (typically left, right, top, bottom for petri dishes). The captured coordinates are saved directly to the microscope YAML file and the map reloads automatically on save. Only inserts with editable reference points in the YAML configuration can be calibrated this way (the default single-slide insert is synthesized from the stage limits and is not calibratable here -- edit `slide_size_um` / `stage.limits` for that).
