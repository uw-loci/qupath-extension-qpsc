...
| Calibrate... | Button | — | **New:** Calibrate the selected insert's reference points by driving the stage to each point in Live View and capturing the position. Opens a **non-modal** window (it stays open and does **not** block the Live Viewer / joystick, so you can keep moving the stage); press "Use current" to capture each axis (e.g. well-edge aperture left/right/top/bottom for petri-dish carriers). "Save to config" writes the values straight back to the microscope YAML and reloads the map; the window stays open so you can capture more points. Only enabled for inserts that have editable reference points in the config (the synthesized single-slide baseline cannot be calibrated this way). |
