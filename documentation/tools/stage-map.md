...
| Camera View / Stage View | CheckBox | From config | Chooses how the Stage Map is oriented. **Checked = "Camera View"**: oriented like the Live Viewer and the acquisition image (what the camera sees); this is camera orientation XOR optical flip. **Unchecked = "Stage View"**: oriented like the slides as they physically sit on the stage (scope type + slide insertion, independent of optics). The checkbox label changes with its state. Independent of which project image is open. Updates automatically when stage polarity or camera orientation are edited (e.g., via Calibrate Directions). Toggle to compare the two views. |
| Scope type | ComboBox | Upright | **Orientation control (factor 1a)** — whether the objective faces up (Upright) or down (Inverted). Part of the per-microscope hardware description that determines how the slide physically sits on the stage. Affects Stage View only. Read from and written to the microscope config's `light_path: scope_type` block. Defaults to Upright (identity orientation). |
| Slide insertion | ComboBox | A | **Orientation control (factor 1b)** — the two ways a slide drops into the holder (slide can be inserted either way, differing by an in-plane 180-degree rotation). Affects Stage View only. Read from and written to the microscope config's `light_path: slide_insertion` block. Defaults to A (identity orientation). |
| Optics | ComboBox | none | **Orientation control (factor 2)** — objective + tube-lens parity (e.g., PPM's objective inversion = 180/XY; OWS3 net camera-vs-stage flip). Affects Camera View only (display only; does not touch acquisition). Read from and written to the microscope config's `light_path: optical_flip` block. Defaults to none (identity orientation). |
