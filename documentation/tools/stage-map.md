...
| Slide | ComboBox | From config | How the slide is placed in the holder: **Label left (as scanned)** or **Label right (180)**. Controls the Stage View rotation to match the slide's physical orientation on the stage. Saved per-run. The set-once orientation properties (scope type: upright/inverted, optical flip: objective+tube parity) are edited in **Utilities > Microscope Configuration > Light Path Orientation...**. |
