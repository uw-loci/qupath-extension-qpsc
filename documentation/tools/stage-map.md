...
| Orientation | MenuButton | — | Opens a dropdown menu containing three per-microscope orientation controls (configured once at setup, rarely edited). Keeps the toolbar readable by grouping related controls. **Scope type** (ComboBox, default Upright) — whether the objective faces up or down. **Slide insertion** (ComboBox, default A) — which way the slide drops into the holder. **Optics** (ComboBox, default none) — objective + tube-lens parity affecting Camera View. All read from and written to the microscope config's `light_path` block. |
