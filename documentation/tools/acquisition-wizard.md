...
| White Balance | Green/Orange (per mode) | Calibrate... | Launches the white balance calibration workflow. On a JAI camera each calibratable WB mode gets **its own dot and colour**: Green when that mode is calibrated, Orange when it is not. Camera AWB is not listed — the camera does it itself and there is nothing on our side to check. Gray on a non-JAI camera, where white balance is configured through MicroManager instead. This row reports only whether each mode is **calibrated**; whether its *backgrounds* are still valid is the Background Correction row below. |
| Background Correction | Green/Orange (per mode) | Collect... | Launches the background/flat-field image collection workflow. On a JAI camera each white-balance mode that needs backgrounds gets **its own dot and colour**: Green when that mode's backgrounds are valid, Orange when they are stale (collected before the last WB calibration) or absent. So per-angle can read green while simple reads orange — which is the point: it tells you the mode you are about to acquire with is fine. A monochrome camera or a per-channel fluorescence profile shows a single status instead, since neither splits by WB mode. |
