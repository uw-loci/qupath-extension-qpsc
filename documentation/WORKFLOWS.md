...
4. Run the slots. There are three ways, and you can mix them:
   ...
   - Or drive one slot by hand with its row buttons: **Open** switches to that slot's macro entry, **Run Single** launches the regular workflow on it, **Done** marks the slot as completed by hand (e.g. you acquired it outside the batch), and **Skip** excludes it from the run. (Entry names display without the rotation/flip suffix for clarity; hover over them to see the full name.)
