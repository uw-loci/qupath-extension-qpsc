...
**Alignment during multi-slide runs:** During all multi-slide batch passes (Run All Remaining, Set Up All Remaining, or Acquire All Set-Up), each slide's alignment is **always fresh** (never reused from a saved per-slide alignment or the SIFT/preset path). This is by design: a slide's position in the holder depends on its current mount (slot location, orientation) and a prior alignment from a single-slide layout is meaningless for the holder.
   ...
   - The workflow ALWAYS runs the complete 3-point manual landmark alignment (three measured points establish the slide's true position AND rotation). The holder's per-slot center calibration provides only a rough auto-move to position the stage near the first reference tile.
   - **If "Single-tile refinement" is selected** (recommended for multi-slide): after the 3-point landmark completes, the single-tile refinement step refines the alignment on a reference tile (SIFT auto-alignment or manual) and captures focus Z for unattended acquisition.
   - **If "Full manual" is explicitly selected**: the workflow runs the complete 3-point manual landmark, then visits two additional confirmation tiles (tiles 2 and 3) for final refinement, same as single-slide mode.
