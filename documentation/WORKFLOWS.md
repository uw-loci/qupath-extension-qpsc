...
4. Run the slots. The batch panel is organized into collapsible sections: **Slots** (per-slide table), **Alignment**, **Refinement**, and **Advanced / SIFT settings**. The slots table shows each slide's entry, rotation, current status, and per-row actions. The **Alignment** section shows Stage Map view controls and autofocus-on-slot-jump status (read-only during batch runs). The **Advanced / SIFT settings** section lets you toggle autofocus-on-slot-jump, auto-Camera-View, auto-zoom-to-tissue, SIFT auto-align trust, and SIFT confidence threshold. There are three ways to run the batch, and you can mix them:
   ...
   **Control buttons:**
   - **Run All Remaining**, **Set Up All Remaining**, **Acquire All Set-Up** — Drivers for the three batch execution modes. All three respect the "Stop after current slide" checkbox.
...
**Alignment during multi-slide runs:** By default, during all multi-slide batch passes (Run All Remaining, Set Up All Remaining, or Acquire All Set-Up), each slide's alignment is **always fresh** (never reused from a saved per-slide alignment created for a different stage insert). This is by design: a slide's position in the holder depends on its current mount (slot location, orientation) and a prior alignment from a single-slide layout is meaningless for the holder.
   ...
   **How fresh alignment is created** (when reuse is not enabled):
   ...
   - **Refinement after primary/fallback:** A refinement step follows the initial alignment to correct the per-slot position and capture focus Z for unattended acquisition. **Single-tile** corrects only the offset (fast, but cannot fix a slide that sits rotated in its slot). **Multi-tile** (recommended when the holder lets slides shift/rotate) captures 2+ spread reference tiles and solves a rotation + scale correction, keeping the alignment accurate across the whole slide. **Full manual** re-aligns from scratch. If acquisition is accurate near one spot but drifts off with distance after a single-tile refine, switch that slot to multi-tile.
   ...
   **What you see:** When the workflow starts multi-slide batch acquisition and detects that a saved alignment was made for a different insert, a message appears in the Refinement Options section recommending single-tile refinement and explaining: "[i] Recommendation: Single-tile refinement -- this slide is aligned fresh for the holder; any saved alignment was for a different insert and is not used. Single-tile verifies the alignment and captures the focus depth for unattended acquisition."
