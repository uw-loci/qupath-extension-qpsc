...
4. Run the slots. The batch panel is organized into collapsible sections: **Slots** (per-slide table), **Alignment**, **Refinement**, and **Advanced / SIFT settings** (placeholder sections for future increments). The slots table shows each slide's entry, rotation, current status, and per-row actions. There are three ways to run, and you can mix them:
   ...
   **Control buttons:**
   ...
   - **Abort All** — Click this button (styled in red in the footer) to stop the batch driver immediately and prevent further slides from starting. A currently running acquisition still completes or cancels via its own dialog, but no new slides will be opened after this button is clicked. The Abort All button remains reachable even during an active acquisition.
