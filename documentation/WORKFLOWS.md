...
|--------------|----------|-----------|
| Scan a rectangular region by stage coordinates | [Bounded Acquisition](#workflow-1-bounded-acquisition) | Extensions -> QP Scope -> Acquisition controls -> Bounded Acquisition |
| Acquire high-res images of annotated regions on an overview slide | [Acquire from Existing Image](#workflow-2-acquire-from-existing-image) | Extensions -> QP Scope -> Acquisition controls -> Acquire from Existing Image |
| Acquire multi-channel widefield immunofluorescence (IF, BF+IF, or LC-PolScope) | [Multi-Channel Acquisition](#multi-channel-acquisition-widefield-if-bfif) | Same menu entries as Bounded / Existing Image (under Acquisition controls) |
...
**Single-tile refinement:** When selected, QPSC acquires one reference tile and shows you the result so you can verify alignment before scanning all annotations. This catches minor drift without a full re-alignment. Note: single-tile corrects only the *offset* (translation) -- it re-anchors the alignment at the one tile you refine but cannot correct a *rotation*. If a slide has shifted or rotated in its holder slot, the alignment will be accurate at the refined tile but drift further off with distance from it.

**Multi-tile refinement:** When a slide may have rotated in its holder slot (common with the multi-slide vertical holder), choose multi-tile refinement. You add 2 or more reference tiles spread across the slide; for each, the stage moves to its predicted position and an embedded capture pane appears where SIFT (or a manual nudge) captures where it actually is. QPSC then solves a rotation + scale correction from all the points -- not just an offset -- so the alignment stays accurate across the whole slide. The refinement panel shows the recovered rotation and scale live as diagnostics (a non-zero rotation confirms slide play; a scale far from 1.0 hints at a pixel-size/objective mismatch). If the correction appears implausibly large (rotation > 10° or scale < 0.9 / > 1.1), a red warning appears indicating the base alignment does not match the stage — investigate before saving. Spread the points far apart and avoid a straight line for the best rotation estimate. This is faster than a full manual re-alignment because it starts from the existing alignment.
...
3. For each slot, assign one project macro entry and set its **Rotation** to match how the slide is physically mounted:
   ...
   - **Live preview:** Open the Stage Map beside this dialog to see all assigned slides at their chosen rotations rendered over the holder's slots. Adjust rotations in real time and watch the Stage Map preview update live, so you can verify placement before acquisition starts. The preview remains visible after the dialog closes, serving as a layout reference through the per-slide alignments that follow.
   - Check the box for "Skip" to leave a slot unoccupied.
4. Run the slots. The batch panel is organized into collapsible sections: **Slots** (per-slide table), **Alignment**, **Refinement**, and **Advanced / SIFT settings**. The slots table shows each slide's entry, rotation, current status, and per-row actions. The **Alignment** section shows Stage Map view controls and autofocus-on-slot-jump status (read-only during batch runs). The **Advanced / SIFT settings** section lets you toggle autofocus-on-slot-jump, auto-Camera-View, auto-zoom-to-tissue, SIFT auto-align trust, and SIFT confidence threshold. The controls are **color-coded by mode**: the **Slots** table is framed in **green** ("one slide at a time" — the per-row manual controls), and the automated drivers are grouped in a **blue** "AUTOMATED RUN" frame at the bottom of the panel. 
   
   **Collapse the panel during alignment:** The panel's banner bar includes a **Collapse** button. Click it to shrink the panel to a thin bar so it doesn't cover your alignment views (Stage Map, refinement capture pane, etc.). The window stays draggable and closable, and the **Abort Batch** button remains accessible from the refinement dialog. Click **Expand** on the bar to restore the full panel. This is useful during interactive alignment and refinement steps when you need maximum screen real estate for the Stage Map and other dialogs. **Note:** The panel also auto-collapses when you click into another window during the Setup phase, so it stays out of your way during interactive dialogs. This auto-collapse is disabled during the unattended Acquire phase so the progress panel remains visible while slides are being acquired.

   **Dialogs dock beside the panel:** During a batch, each per-slot dialog (the acquisition-config dialog, green-box preview, tile selection, and refinement) opens **docked beside the progress panel** — on whichever side has room to the screen edge — instead of stacking on top of the alignment views. This is only the *initial* placement: if you use the Dialog Position Manager extension, any dialog you have moved keeps its saved position (the dock only seeds first-time placement). Move a dialog wherever you like and, with the Dialog Position Manager installed, it will return there next time.
   
   **Estimated run time:** The panel displays a live estimate of how long the whole-batch acquisition will take. Before any slides are set up, the estimate shows "Estimated run time: set up slides to see an estimate." Once you complete **Step 1: Set Up All Remaining**, each slot's acquisition settings and annotations are finalized, and the estimate computes and displays:
   - **During setup and before acquisition:** Shows total slides, total tiles, and includes a qualifier "(measured timing)" or "(rough -- no measured timing yet)".
   - **During the unattended acquire pass (Step 2):** The estimate **updates live** as each slide finishes — a "Remaining ~Xs of ~Ys" line shows the shrinking predicted time for slides not yet done, the count of remaining slides and tiles, and the same timing qualifier.
   - **After all acquisitions complete:** Shows "Run complete" and the final total time and tile count.
   
   The estimate is **annotation-aware**: it accounts for the fact that many small regions cost more than one large region at equal tile count. Each annotation (region) incurs a startup overhead (region move + focus pass, ~6 seconds), so an acquisition with 20 small 5-tile regions costs noticeably more than one 100-tile region, even though both acquire 100 tiles. The estimate uses the learned per-file wall-clock cost when available (which self-calibrates after every run), plus per-annotation and per-slide startup overhead. No measured timing yet? The estimate uses a conservative fallback and is labeled "rough -- no measured timing yet" until at least one full acquisition run completes on this microscope.

   Whenever the panel is idle, an **attention pulse glows the recommended next step** (Step 1 → Step 2 → Finish) so you always know what to click next. There are two ways to run the batch, and you can mix them:
   - **Two-pass (unattended acquire)** -- for a walk-away batch, follow the two numbered steps in the blue AUTOMATED RUN frame. First click **Step 1: Set Up All Remaining**: this runs the interactive align + refine + tissue pass on every slot *without acquiring*, so each slot advances to **Set up (ready to acquire)** and remembers its acquisition settings. Then click **Step 2: Acquire All Set-Up**: the panel acquires every set-up slot **unattended, with no dialogs**, replaying each slot's settings against the alignment saved during setup. The acquire pass **pipelines over stitching**: each slot advances to the next as soon as its acquisition (stage work) completes, while stitching and image import happen in the background. This allows overlapped stitching and faster batch throughput — later slots start acquiring while earlier slots are still stitching. The batch is fully complete when all slots have finished acquiring and all stitching has settled. **Alerts during acquisition:** Per-slide completion alerts (system beep + toast notification) are sent when each slide's acquisition finishes. An additional batch-complete push notification is sent once all stitching is done. (Push notifications require ntfy.sh to be configured in Communication Settings; see [Alerts](PREFERENCES.md#alerts-qupath-scope-alerts).) **Saturation summary:** After all acquisitions and stitching complete, a combined saturation summary dialog appears (if any tiles had concerning saturation). The dialog lists every acquisition with saturation flagged at the top (worst-first), shows a scrollable per-sample list (flagged samples expanded by default, clean ones collapsed), and allows you to double-click tiles to navigate the stage to them. If no concerning saturation was detected across the entire run, no dialog appears. Stitched images are added to the project but not automatically opened in the viewer during the run (so a background import cannot yank the viewer away from the slide currently being processed); open them from the Project browser, which refreshes once the batch finishes. Front-load all your decisions in the setup pass, then leave it running. **Tip:** choose **single-tile refinement** during each slot's setup -- the focus Z it establishes is carried into the unattended acquire pass to seed that slot's first-tile autofocus, so acquisition starts near focus instead of hunting. (With no refinement, the acquire pass still autofocuses from the current stage Z; it just has no head-start.)
   - Or drive one slot manually: Use the **Status** dropdown to set each slot to **Done** (if you acquired it outside the batch) or **Skipped** (to exclude it). Click **Open** to switch to that slot's macro entry. Right-click a slot's entry name to see **Run Single**, which launches the regular workflow on that slot. (Entry names display without the rotation/flip suffix for clarity; hover over them to see the full name.)

   **Control buttons:**
   - **Step 1: Set Up All Remaining** and **Step 2: Acquire All Set-Up** — the two-pass batch drivers, grouped in the blue AUTOMATED RUN frame; both respect the "Stop after current slide" checkbox. The idle attention pulse points you to whichever step is next.
   ...
   - **Abort All** — Click this button (styled in red in the footer) to stop the batch driver immediately and prevent further slides from starting. When no acquisition is in flight, this immediately closes the batch panel. When an acquisition IS running, clicking Abort All shows a confirmation dialog; if you confirm, the running acquisition is cancelled (the CANCEL command is sent to the microscope, tiles already captured are kept, and the current tile is discarded). The button displays "Cancelling..." while waiting for the acquire loop to unwind, then "Aborted" once the acquisition has finished unwinding. (If the microscope does not confirm the cancel within a timeout, a warning advises you to verify the stage state -- QPSC sends the cancel but does not itself guarantee the hardware lands in a safe mid-tile state.) No further slides will be opened after this button is clicked. The Abort All button remains reachable even during an active acquisition. During an interactive alignment refinement, the refinement dialog also carries an **Abort Batch** button (it can otherwise cover the panel's Abort All): clicking it keeps the current slide's alignment, closes the dialog, and trips the same batch abort.
   
   A slot advances to **Done** on a successful acquisition, or stays **In Progress** / **Set up** (so you can retry or manually set to Skipped) if a run is cancelled at a gate or hits a handled error.
5. Once every slot is Done or Skipped, click **Finish**. A combined saturation summary is shown (if any tiles had concerning saturation during the run).

**Alignment during multi-slide runs:** By default, during all multi-slide batch passes (Set Up All Remaining or Acquire All Set-Up), each slide's alignment is **always fresh** (never reused from a saved per-slide alignment created for a different stage insert). This is by design: a slide's position in the holder depends on its current mount (slot location, orientation) and a prior alignment from a single-slide layout is meaningless for the holder.

   **TEST-ONLY alignment reuse:** When the preference **Reuse saved alignment (TESTING ONLY)** is enabled, the batch asks for one-time confirmation and then reuses each slot's saved per-slide alignment instead of re-deriving it (skipping alignment and single-tile refinement). **This is UNSAFE for real acquisition** — it assumes every slide is still physically mounted exactly as when its alignment was captured; any slide remount invalidates the reused transform. Slots with no valid saved alignment automatically fall back to fresh alignment. See [Preferences: Reuse saved alignment (TESTING ONLY)](PREFERENCES.md#reuse-saved-alignment-testing-only) for details and when to enable it.

   **How fresh alignment is created** (when reuse is not enabled):
   - **Primary path (when available):** The workflow re-detects tissue location on the slide using the green-box + scanner-preset path. The scanner preset provides optical orientation/scale (calibrated at setup time), and per-slide re-detection ensures the tissue is correctly located on each slide in the holder. The holder's per-slot center calibration provides a rough auto-move to position the stage near the first tile.
   - **Fallback path (when no preset available):** If no usable scanner preset exists, the workflow falls back to the complete 3-point manual landmark alignment (three measured points establish the slide's true position AND rotation).
   - **Refinement after primary/fallback:** A refinement step follows the initial alignment to correct the per-slot position and capture focus Z for unattended acquisition. **Single-tile** corrects only the offset (fast, but cannot fix a slide that sits rotated in its slot). **Multi-tile** captures 2+ spread reference tiles and solves a rotation + scale correction, keeping the alignment accurate across the whole slide. **Full manual** re-aligns from scratch. For a **multi-slide vertical holder** (e.g. `quad_v`), each slide sits loosely in a tall slot and can rotate, so the dialog now **defaults to Multi-tile** for these holders; single vertical slides and horizontal inserts still default to single-tile. If acquisition is accurate near one spot but drifts off with distance after a single-tile refine, switch that slot to multi-tile.
   - **Autofocus on each refinement jump:** When you select (or the workflow moves to) a refinement tile, the stage jumps to that tile's predicted position and then **autofocuses before the refinement dialog/capture pane appears**, so the live view is already in focus when you arrive. This honors the same **Autofocus on slot jump** toggle (Advanced / SIFT settings section, default on) as the between-slot jump; turn it off to focus by hand. The autofocus uses your configured Live Viewer method: with **Streaming** (default) it runs a full focus **search** while the Live Viewer stream stays open — the right choice for slot jumps, which can land far from the previous slide's focus. It falls back to the **Sweep** drift-check only when the method is set to Sweep or no Live Viewer stream is open; note Sweep is a narrow-range drift check and can report success without finding focus if the stage starts far from focus, so keep a Live Viewer stream open during multi-slide alignment.
   - **Numbered alignment steps (multi-tile):** The multi-tile refinement panel lists the steps in order, each color-matched to its button: **1. Select tile** (blue) moves the stage to a tile you pick; **2. Auto-Align (SIFT)** (amber) snaps onto it — a link opens the alignment docs and the SIFT **Settings...** if it struggles, and **2b** lets you nudge via the Stage Map and re-run SIFT; **3. Add reference point** (teal) records the point once the live view matches; **4. Solve & Save** (green) becomes available with 2+ points. An attention pulse glows the next action — the panel points you to **Select tile** / **Solve**, and inside a point it moves from **Auto-Align (SIFT)** to **Add reference point** once SIFT lands a match. Repeat 1–3 per point, spreading points far apart (not in a line) for the best rotation estimate.
   - **Stage Map view on refinement start:** When the first refinement tile is selected, the Stage Map automatically switches to **Camera View** and zooms to the **tissue bounding box** (if one is set), per the **Force Camera View on alignment start** and **Zoom Stage Map to tissue on alignment start** preferences (both enabled by default in Advanced / SIFT settings). If the Stage Map is closed or no bounding box is previewed, these adjustments are silently skipped. Turn off either toggle if you prefer manual Stage Map control during refinement.

   The setup pass always creates a fresh per-slide alignment JSON specific to that slide's position in the current holder; the acquire pass then replays that JSON. The system records which stage insert each alignment was made on. If you have a saved alignment from a prior run and switch to a different stage insert (e.g., from a single-slide layout to a multi-slide holder), the workflow detects the insert mismatch and treats the old alignment as invalid. Even if an old alignment JSON exists, the dialog will recommend "Single-tile refinement" with a message explaining that the alignment is not being used because it was built for a different insert. This safety mechanism prevents accidental use of mismatched alignments.

   **What you see:** When the workflow starts multi-slide batch acquisition and detects that a saved alignment was made for a different insert, a message appears in the Refinement Options section recommending fresh refinement. For a **multi-slide vertical holder** it recommends multi-tile: "[i] Recommendation: Multi-tile refinement -- this slide is aligned fresh in a multi-slide vertical holder, where slot play adds rotation and scale that single-tile (translation-only) cannot correct. Multi-tile solves rotation + scale from 2+ tiles and also captures the focus depth for unattended acquisition." For other inserts it recommends single-tile: "[i] Recommendation: Single-tile refinement -- this slide is aligned fresh for the holder; any saved alignment was for a different insert and is not used. Single-tile verifies the alignment and captures the focus depth for unattended acquisition." Either recommendation is a default you can override per slide.
