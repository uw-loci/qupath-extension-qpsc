...
4. Run the slots. The batch panel is organized into collapsible sections: **Slots** (per-slide table), **Alignment**, **Refinement**, and **Advanced / SIFT settings**. The slots table shows each slide's entry, rotation, current status, and per-row actions. The **Alignment** section shows Stage Map view controls and autofocus-on-slot-jump status (read-only during batch runs). The **Advanced / SIFT settings** section lets you toggle autofocus-on-slot-jump, auto-Camera-View, auto-zoom-to-tissue, SIFT auto-align trust, and SIFT confidence threshold. The controls are **color-coded by mode**: the **Slots** table is framed in **green** ("one slide at a time" — the per-row manual controls), and the automated drivers are grouped in a **blue** "AUTOMATED RUN" frame at the bottom of the panel. There are three ways to run the batch, and you can mix them:
   ...
   **Control buttons:**
   - **Step 1: Set Up All Remaining**, **Step 2: Acquire All Set-Up**, and **Run All Remaining** (one-pass alternative) — Drivers for the three batch execution modes, grouped in the blue AUTOMATED RUN frame. All three respect the "Stop after current slide" checkbox.
...
**Alignment during multi-slide runs:** By default, during all multi-slide batch passes (Run All Remaining, Set Up All Remaining, or Acquire All Set-Up), each slide's alignment is **always fresh** (never reused from a saved per-slide alignment created for a different stage insert). This is by design: a slide's position in the holder depends on its current mount (slot location, orientation) and a prior alignment from a single-slide layout is meaningless for the holder.
   ...
   **How fresh alignment is created** (when reuse is not enabled):
   ...
   - **Numbered alignment steps (multi-tile):** The multi-tile refinement panel lists the steps in order, each color-matched to its button: **1. Select tile** (blue) moves the stage to a tile you pick; **2. Auto-Align (SIFT)** (amber) snaps onto it — a link opens the alignment docs and the SIFT **Settings...** if it struggles, and **2b** lets you nudge via the Stage Map and re-run SIFT; **3. Add reference point** (teal) records the point once the live view matches; **4. Solve & Save** (green) becomes available with 2+ points. Repeat 1–3 per point, spreading points far apart (not in a line) for the best rotation estimate.
