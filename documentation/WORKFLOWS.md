...
**Alignment during multi-slide runs:** During all multi-slide batch passes (Run All Remaining, Set Up All Remaining, or Acquire All Set-Up), each slide's alignment is **always fresh** (never reused from a saved per-slide alignment or the SIFT/preset path). This is by design: a slide's position in the holder depends on its current mount (slot location, orientation) and a prior alignment from a single-slide layout is meaningless for the holder.

   How the fresh alignment is created depends on your refinement choice:
   - **If "Single-tile refinement" is selected** (recommended for multi-slide): the holder's per-slot center calibration seeds the starting transform, so selecting a reference tile auto-moves the stage near the tissue. The single-tile refinement then refines that estimate (SIFT auto-alignment or manual) instead of running a full 3-point manual landmark.
   - **If "Full manual" is explicitly selected**: the workflow runs the complete 3-point manual landmark alignment, ignoring the slot-center seed.
