...
Therefore `StageMapWindow.resolveCurrentFlipAxes` returns **`StageImageTransform.cameraFlipFlags()`** (camera only) — it must **not** use `stitcherFlipFlags()` (stage polarity + camera), which would re-apply the polarity already baked into the map geometry, and it must **not** fold in the source preset's `flipMacroX/Y` (see the next paragraph).
...
**The macro-overlay flip is applied to the macro image, not to the whole-canvas mirror (2026-08-05 fix).** `flipMacroX/Y` is the flip that brings the *source scanner's macro image* into agreement with the **live camera view** on the target microscope — that is exactly what the `(flipped X|Y|XY)` sibling exists for (see `ImageFlipHelper`: "on a flipped scope the unflipped base and the live camera view disagree by a mirror"). It is a property of the `(sourceScanner, targetMicroscope)` pair, not a camera-vs-stage relationship, so it belongs on the macro overlay pixels alone, via `StageMapWindow.resolveMacroPresetFlip()` at the point `MacroImageUtility.flipMacroImage` is called. An earlier revision XOR'd `flipMacroX/Y` into `resolveCurrentFlipAxes` (the whole-canvas mirror). That rotated the insert/slides/crosshair/FOV box out of *both* the stage and camera frames whenever a source preset with a macro flip was selected: on OWS3 the `Ocus40` source preset has `flipMacroX = true`, so selecting it auto-checked "Apply Flips" and X-mirrored the entire Stage Map even though the camera and stage were unflipped (reported 2026-08-05). Splitting the two flips fixes that: the whole canvas gets camera flip only, and the macro overlay gets its preset flip, then the canvas camera flip carries the placed overlay to the displayed frame.
...
Skipped with a written reason instead of guessed: rotated working entries (base -> `(rotated N)` frame is not exported yet), JSONs with no recorded microscope, sub-acquisitions with no FOV or no `acquired_on_microscope`. Design and schema findings: `claude-reports/design/2026-09-17_ngff-0.6-spatial-scene-export.md`.
...
> **The matched-offset move — it is just `−offset`, no rig transform (final: `60b406f1`, 2026-08-09).** The caveat above predicted a real second-order bug, and it fired: a high-confidence match moved the stage the wrong way.
>
> - The server (`sift_matcher.py`) returns the offset as *microscope-center − WSI-center*, and the WSI region is centered on the target tile, so the offset points **from the target to the current camera position**. The stage must move the opposite way: `target − current = −offset`.
> - **No light-path / alignment transform is applied.** SIFT computes the offset by matching the WSI region against the **live camera image**; the match only succeeds when the two share an orientation, so the match itself has already resolved the light path. `currentPos` and the offset are captured together, so a *relative* "close the gap" correction is already in the stage-command frame. `−offset` is the correct move on **every** rig — verified on both PPM (composite identity) and OWS3 (composite INVERT_XY, ground truth 2026-08-09), and it is the only operation consistent with both.
>
> A prior attempt (`faf94e85`) mapped the offset through the per-slide `fullResToStage` alignment transform. That transform's linear part is **identity on PPM** (the flip lives in the sibling pixels), so composing was harmless and PPM verified — but on **OWS3** the flip lives in the transform (XY-flip), so composing turned `−offset` into `+offset` and mirrored the move (SIFT moved `(−80.9, 174.6)`; correct was `(+148.7, −201.4)`). The transform + its `fullResToStage` plumbing were removed. `SiftAutoAlignHelper.composeSiftOffsetToStageDelta(ox, oy, serverFlipX, serverFlipY)` now = undo server flip → negate. Unit-tested in `SiftOffsetComposeTest`. Log: `SIFT move: … stageDelta=(…) [-offset]`.
>
> The one case that could still need more is a scope with a genuine camera **rotation** (not just a flip); catch it by matching there, and it would use `StageImageTransform` (like click-to-center), not the alignment transform. Full write-up: `claude-reports/2026-08-08_sift-auto-move-offset-direction.md`.
