...
> **The matched-offset move — direction + orientation (fixed 2026-08-08, `faf94e85`).** The caveat above predicted a real second-order bug, and it fired on PPM: a high-confidence match moved the stage the wrong way. Two facts govern the move, and both were mishandled:
>
> 1. **Direction (rig-invariant).** The server (`sift_matcher.py`) returns the offset as *microscope-center − WSI-center*, and the WSI region is centered on the target tile, so the offset points **from the target to the current camera position**. The stage must therefore move by the **negation** (`target − current = −offset`). The old code added it and drove to the mirror landing. This negation is a property of the server's return format, identical on every rig — **not** a per-scope value, so it is *not* in the light-path/stage model. Proven on PPM: predicted `P`, `+offset` landed at `P+offset`, the true tile was at `P−offset`, and "Go to centroid" (= `P`) sat at their exact midpoint.
> 2. **Orientation (per-rig).** The negated offset is a displacement in the WSI-*entry* image frame; it is mapped to a stage displacement through the **same** `fullResToStage` alignment transform that predicts the tile position (`deltaTransform`, linear part only). Note this is **not** `StageImageTransform` (that maps the *live camera* frame, used by click-to-center); the SIFT offset is in the *entry* frame. On PPM this linear part is identity (the flip lives in the sibling pixels), so the whole correction reduces to the negation; on an XY-inverted alignment it becomes `+offset` — which is *why* the raw code ever worked on OWS3.
>
> Implemented in `SiftAutoAlignHelper.composeSiftOffsetToStageDelta` (undo server flip → negate → um-to-px → `deltaTransform`); the transform is threaded into `autoAlign` from every SIFT caller. Unit-tested in `SiftOffsetComposeTest`. Full write-up: `claude-reports/2026-08-08_sift-auto-move-offset-direction.md`. **OWS3 re-verification owed** — the `SIFT move:` log line shows `stageDelta` vs the old raw for the case check.
